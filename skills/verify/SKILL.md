...
Orchestrator role: validate preconditions, dispatch `verifier`, surface result + next-step suggestion.
</objective>
...
2. **Dispatch `verifier`** with the phase number. The verifier owns all AskUserQuestion calls for the UAT itself.

3. **Report** the verifier's ≤200-word summary. Suggest:
   - PASS → `/psd:ship {N}`
   - FAIL/PARTIAL → `/psd:plan {N} --from-failure`, then `/psd:execute {N}`, then re-verify

Preserve all gates: never fabricate a pass; never advance STATE beyond `last_skill: verify` here (that's ship's job).
</process>
