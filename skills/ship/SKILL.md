...
Orchestrator role: validate VERIFICATION.md PASS gate, dispatch `shipper`, surface PR/branch info.
</objective>
...
2. **Dispatch `retrospector`** for Phase {N}. It extracts ≤5 obstacles into `phases/Phase {N}/LESSONS.md` and appends to `.planning/LESSONS.md` "## Active". 60s timeout. On error/timeout, proceed; pass `lessons_status=skipped:<reason>` to the shipper. See `@$HOME/.claude/workflows/retrospect.md`.

3. **Dispatch `shipper`** with the phase number and the retrospector's status. Shipper includes the new LESSONS.md files in the `chore(planning): ship phase {N}` commit and reports the lessons-captured count in its quality-gates block.
