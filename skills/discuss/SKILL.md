...
<objective>
Capture phase-level decisions, constraints, and edge-case handling BEFORE planning. Adaptive Q&A — 3-7 targeted questions on what's unclear about the phase. Output: `phases/Phase {N}/CONTEXT.md` consumed by `plan`.

Orchestrator role: validate state, resolve phase number, gate against accidental overwrite, dispatch `discusser`, surface result.
</objective>
...
2. **Companion check (suggestion mode):** look at PROJECT.md's Stack section and Phase {N}'s goal in ROADMAP.md.
   - If goal/Stack mentions AI/LLM/agent/embedding/RAG AND `gsd-ai-integration-phase` is in the available-skills list, print:
     > FYI: `gsd-ai-integration-phase` is also available — produces a heavier AI-SPEC contract with full eval rigor. Run it directly for AI-critical phases; otherwise continuing with `discuss` (lightweight AI-SPEC.md).
