...
<objective>
Execute each atomic plan in the specified phase. One `psd-executor` subagent per plan, run in fresh contexts; plans within a wave run in parallel; waves run sequentially. One commit per plan.
...
2. **Companion check (suggestion mode):** scan the available-skills list. If `superpowers:using-git-worktrees` is listed AND the phase has multiple waves OR appears risky (touches auth/payments/data-migration files per plan `files:` lists), print:
   ...
   > FYI: `superpowers:using-git-worktrees` is also available — runs the work in an isolated worktree so you can compare against current main. Recommended for risky phases. Abort and use it directly if helpful; otherwise continuing with `psd-execute` (in current branch).
...
4. **For each wave (sequential):**
   - Dispatch one `psd-executor` per plan in the wave **in parallel** (multiple Task calls in one assistant message).
