...
Orchestrator role: validate state, resolve phase number, gate against accidental overwrite, dispatch `psd-planner`, summarize result.
</objective>
...
3. **Dispatch `psd-planner`** with the phase number and any flags. Default behavior (accuracy-first):
