...
Note: `psd-init` and `psd-plan` ALSO call this previewer automatically as their final step (with an approval gate). This skill is for ad-hoc re-viewing.
</objective>
...
2. **Dispatch `psd-previewer`** with the mode. Subagent reads ROADMAP/PLAN/PROJECT itself and returns the narrative.
