...
Orchestrator role: validate env (via DOCTOR.md), surface companion skills, dispatch `psd-deployer`. Deploy logic + smoke test happens in the subagent.
</objective>
...
2. **Companion check (suggestion mode):** if `vercel:deploy` is in the available-skills list, print:
   ...
   > FYI: `vercel:deploy` is also available — direct Vercel deploy. Continuing with `psd-deploy` (which orchestrates around it for smoke-testing and recording).
...
4. **Dispatch `psd-deployer`** with `target` (`preview`/`production`) and `phase` (from `--phase N` if passed, else `none`).
