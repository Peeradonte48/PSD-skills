...
2. **Companion check (suggestion mode):** scan the available-skills list. If `gsd-new-project` is listed, print:
   ...
   > FYI: `gsd-new-project` is also available — deeper project bootstrap with parallel research agents. Run it directly for projects where you want extensive context-gathering; otherwise continuing with `psd-init` (lean, batteries-included defaults).
...
4. **Dispatch `psd-initializer`** with only the gathered answers + cwd. Do not paste workflow contents into the prompt — reference @$HOME/.claude/workflows/init.md.
