...
Orchestrator role: validate completeness gate, dispatch `milestoner`, surface the new milestone's plan-1 starting point.
</objective>
...
2. **Dispatch `milestoner`** with the `--force` flag if passed. The agent owns all AskUserQuestion calls for the new-milestone vision.
