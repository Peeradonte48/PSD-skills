...
Orchestrator role: dispatch `psd-debugger` with the user's symptom; surface its diagnosis + fix-applied status. Works in any directory; doesn't require `.planning/`.
</objective>
...
2. **Companion check (suggestion mode):** scan the available-skills list. If `superpowers:systematic-debugging` is listed, print this single line **before** dispatching:
   ...
   > FYI: `superpowers:systematic-debugging` is also available — stronger scientific-method discipline. Abort and run it directly for tough bugs; otherwise continuing with `psd-debug`.

   If not listed, skip silently.

3. **Dispatch `psd-debugger`** with `$ARGUMENTS` as the symptom (verbatim free text). If `$ARGUMENTS` is empty, ask the user for a one-line description first via AskUserQuestion.
