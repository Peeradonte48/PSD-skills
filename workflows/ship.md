...
### Step 1 — `retrospector` (always, before shipper)
...
```
You are retrospector for Phase {N}.
...
6. Stage the LESSONS.md files written by retrospector (phases/Phase {N}/LESSONS.md + .planning/LESSONS.md) into the same `chore(planning): ship phase {N}` commit.
...
`init` scaffolds `.github/pull_request_template.md` for the team. The shipper **fills it in** and passes the result to `gh pr create --body-file`. This way:
