...
## Subagent dispatch
Spawn `psd-initializer` with **only paths and the gathered answers**, not file content. Brief format:

```
You are psd-initializer. Bootstrap a project's .planning/ directory.
...
- **Required CLIs:** <e.g., node 22+, vercel, supabase> (psd-doctor or first-run will check these)
...
1. Dispatch `psd-previewer` in mode `"all"` to generate a plain-English narrative of the just-written ROADMAP.md.
...
3. AskUserQuestion: "Does this match what you want?"
   ...
   - **Revise — change something** → ask what to change (free-text or structured AskUserQuestion); re-dispatch `psd-initializer` with revision feedback to amend ROADMAP.md (and PROJECT.md if needed); loop back to step 1
...
- `STATE.md` shows active_phase=1, last_skill=psd-init
