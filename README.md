...
| `/psd:review [N]` | Lightweight code review + security + eval audit on the phase's diff. Writes `REVIEW.md` with P0/P1/P2. `ship` blocks on uncleared P0s. |
...
Plus an auto-checkpoint hook (`PostToolUse` + `Stop`) that maintains `.planning/CHECKPOINT.md` so `resume` always has a fresh restore point — even after token-limit / API / network errors mid-turn.
...
|---|---|---|---|
| **Opus 4.7** (heaviest reasoning) | `opus` | `researcher`, `planner`, `plan-reviewer`, `discusser`, `brainstormer`, `debugger`, `reviewer` | Accuracy-critical: research synthesis, atomic plan decomposition, adversarial review, scientific-method debugging |
| **Sonnet 4.6** (balanced) | `sonnet` | `executor`, `tester`, `initializer`, `shipper`, `milestoner`, `verifier` | Code generation, template filling, mechanical work where Sonnet's speed/cost balance wins |
| **Haiku 4.5** (fast, cheap) | `haiku` | `resumer`, `previewer`, `retrospector` | Pure read-only summarization with no judgement needed (retrospect: mechanical extraction with mandatory source citation) |
...
Skills are namespaced as `psd:brainstorm`, `psd:plan`, etc. The plugin ships its own `hooks/hooks.json`, so the checkpoint hook auto-registers when the plugin is enabled — no `~/.claude/settings.json` edits.
...
Inside Claude Code, run `/plugin update psd@PSD-skills`.

## Uninstall

`/plugin uninstall psd@PSD-skills`
...
For non-technical users, `init` recommends batteries-included defaults so deployment isn't a science project:
...
├── milestones/
│   └── v1/             # archived after new-milestone
└── phases/
    └── Phase 1/
        ├── PLAN.md
...
`init` scaffolds four root-level files in any project it bootstraps:
...
| `.github/pull_request_template.md` | Structured PR shape used by `ship` and by any teammate opening a PR by hand. |
...
4. A 6-step "Resume without PSD" protocol they (or any AI) can follow to commit the next atomic plan without `execute`
...
- Claude Code
- `jq` (used by the checkpoint hook to safely edit `.planning/CHECKPOINT.md`)
- `git` (recommended; checkpoint hook degrades gracefully without it)
- `gh` CLI (only needed by `ship` to open PRs; not required for the rest)
